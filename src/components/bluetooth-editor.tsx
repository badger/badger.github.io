      ...
      <section className="surface overflow-hidden border-primary/20">
        <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-muted-foreground">Open Editor (BT) on the badge, pair it here, then edit the active secrets.py without Wi-Fi or a server.</p>
          </div>
      ...
      {connected && (view === 'editor' ? (
        <section className="surface p-6 sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div><h2 className="text-xl">secrets.py {dirty && <span className="font-mono text-xs font-normal uppercase tracking-[0.1em] text-amber-300">unsaved</span>}</h2><p className="mt-1 text-sm text-muted-foreground">The badge sends its active file after pairing. Saving writes the badge’s writable override.</p></div>
          ...
          <p className="mt-3 text-xs leading-6 text-muted-foreground">The file travels only between this browser and the selected badge over BLE. A simulator can show the badge UI but cannot validate this radio link.</p>
        </section>
      ) : (
        <section className="surface p-6 sm:p-8">
          <div className="flex items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 text-xl"><Database className="h-5 w-5 text-primary" />Transfer data</h2><p className="mt-1 text-sm text-muted-foreground">Every framed message in this browser session.</p></div><button type="button" onClick={clearLog} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 font-mono text-xs text-muted-foreground hover:border-primary/60 hover:text-foreground"><Trash2 className="h-4 w-4" />Clear</button></div>
          ...
          {log.length ? <div className="mt-5 overflow-x-auto rounded-lg border border-border/60"><table className="w-full min-w-[640px] border-collapse text-left font-mono text-xs"><thead><tr className="border-b border-border/60 bg-background/60 text-muted-foreground"><th className="px-3 py-2 font-normal">Time</th><th className="px-3 py-2 font-normal">Dir</th><th className="px-3 py-2 font-normal">Kind</th><th className="px-3 py-2 font-normal">Bytes</th><th className="px-3 py-2 font-normal">Detail</th></tr></thead><tbody>{[...log].reverse().map((entry) => <tr key={entry.id} className="border-b border-border/40 last:border-0"><td className="px-3 py-2 text-muted-foreground">{formatTime(entry.time)}</td><td className={`px-3 py-2 ${entry.direction === 'tx' ? 'text-primary' : 'text-emerald-300'}`}>{entry.direction === 'tx' ? '→ badge' : '← badge'}</td><td className="px-3 py-2 text-foreground">{entry.kind}</td><td className="px-3 py-2 text-muted-foreground">{entry.bytes}</td><td className="px-3 py-2 text-muted-foreground">{entry.detail}</td></tr>)}</tbody></table></div> : <div className="mt-5 rounded-lg border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Nothing has crossed the Bluetooth link yet.</div>}
        </section>
      ))}
    </div>
  )
